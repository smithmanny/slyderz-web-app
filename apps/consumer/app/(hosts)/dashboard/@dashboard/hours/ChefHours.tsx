          ...
          render={({ field }) => (
            <>
              <Switch
                ...
                {...field}
              />
              <Label htmlFor={`is${days[props.dayIndex]}Enabled`}>{days[props.dayIndex]}</Label>
            </>
          )}
        />
      </div>
...
type HoursSelectType = Omit<HoursType, "createdAt" | "updatedAt" | "calendarId">
...
export default function ChefHours(props: ChefHoursProps) {
  ...
  const form = useSlyderzForm(formSchema, {
    ...
    days: days.map(day => {
      const dayHours = props.calendar.hours.filter(hour => hour.dayOfWeek === day.toLowerCase())

      return {
        dayOfWeek: day,
        hours: dayHours.length > 0 ? dayHours.map(dh => ({
