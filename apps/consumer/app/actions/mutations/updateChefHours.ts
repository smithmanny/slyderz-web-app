...
function buildSelectedHours(entries: { [key: string]: FormDataEntryValue }) {
	const activeHours = [];
	const hourKeys = Object.keys(entries).filter((entry) =>
		entry.includes("hours."),
	);
	...
	for (let x = 0; x < hourKeys.length; x += 2) {
		...
		for (let y = x; y <= x + 1; y++) {
			...
			if (stringToArray[1] && time) {
				const dayOfWeek = buildDaysFromInput(
					stringToArray[1],
				) as DaysOfWeekEnumType;
...
export async function updateChefHoursMutation(input: FormData) {
	...
	const enabledWorkDays = Object.keys(entries).filter((entry) =>
		entry.includes("Enabled"),
	);
	...
	const chefCalendarRes = await db
		.update(calendar)
		.set({
			...workingDays,
		})
		.where(eq(calendar.chefId, chef.id))
