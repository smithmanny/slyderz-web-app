...
export default async function createCheckoutMutation(input: z.infer<typeof CreateCheckoutSchema>) {
  ...
  await prisma.$transaction(async (db) => {
    ...
    const address = await db.address.findFirstOrThrow({ where: {
    ...
    if (order && order.confirmationNumber) {
      ...
      const customerEmailParams = {
        ...
        variables: {
          ...
          orderTotal: String(order.amount + getConsumerServiceFee(order.amount)),
