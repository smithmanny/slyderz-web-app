...
export default resolver.pipe(
  resolver.zod(DestroyMenuItem),
  async (input: any, ctx: Ctx) => {
    const cartItems = ctx.session.cart?.pendingCartItems || [];
    const updatedCartItems: Array<CartItem> = cartItems.filter(
      (item) => item.id !== input.menuItemId
    );
    ...
    await ctx.session.$setPublicData({
      cart: { pendingCartItems: [...updatedCartItems], total: sum },
    });
  }
);
